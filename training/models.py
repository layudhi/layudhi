...
class SocializationExclusion(models.Model):
    employee = models.ForeignKey(Employee, on_delete=models.CASCADE)
    document = models.ForeignKey(Document, on_delete=models.CASCADE, blank=True, null=True)
    event = models.ForeignKey(SocializationEvent, on_delete=models.CASCADE, blank=True, null=True)
    reason = models.TextField(blank=True)
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['employee', 'document'], condition=models.Q(event__isnull=True), name='unique_document_exclusion'),
            models.UniqueConstraint(fields=['employee', 'event'], condition=models.Q(document__isnull=True), name='unique_event_exclusion'),
        ]
        ordering = ['employee__name']

    def __str__(self):
        target = self.document or self.event
        return f'{self.employee} N/A for {target}'
