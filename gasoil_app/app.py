...
from gasoil_app.sources import ONLINE_SOURCE_CHOICES, PUBLIC_SOURCES, ProxyConfig, describe_public_source, fetch_public_source
...
with st.sidebar:
    ...
    source = st.selectbox(
        ...
        format_func=lambda key: PUBLIC_SOURCES[key].label if key in PUBLIC_SOURCES else "CSV URL publik",
        disabled=input_mode != "Ambil online",
    )
    if input_mode == "Ambil online":
        st.info(describe_public_source(source))
...
try:
    ...
    source_df = pd.DataFrame(
        [{**row, "source_price_usd_per_bbl": row.get("mops_usd_per_bbl")} for row in rows]
    )
...
col1.metric("Harga source terakhir", f"{monthly.iloc[-1]['source_price_usd_per_bbl']:.2f} USD/bbl")
col2.metric("Estimasi fuel price terakhir", f"{monthly.iloc[-1]['estimated_fuel_price_idr_per_liter']:,.0f} IDR/l")
col3.metric("Prediksi fuel price akhir", f"{forecast.iloc[-1]['forecast_fuel_price_idr_per_liter']:,.0f} IDR/l")
...
st.line_chart(monthly.set_index("date")[["source_price_usd_per_bbl", "estimated_fuel_price_idr_per_liter"]])

st.subheader("Prediksi")
st.dataframe(forecast, use_container_width=True)
st.line_chart(forecast.set_index("date")[["forecast_source_price_usd_per_bbl", "low_90", "high_90"]])
