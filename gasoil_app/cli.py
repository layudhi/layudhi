...
from gasoil_app.sources import ONLINE_SOURCE_CHOICES, ProxyConfig, describe_public_source, fetch_public_source, source_help_lines
...
def main() -> None:
    ...
    raw_rows = [
        {**row, "source_price_usd_per_bbl": row.get("mops_usd_per_bbl")}
        for row in rows
    ]
    ...
    if not args.csv:
        print(f"Deskripsi sumber: {describe_public_source(args.source)}")
    print(f"Data historis sampai: {latest['date']}")
    print(f"Harga source rata-rata terakhir: {latest['source_price_usd_per_bbl']:.2f} USD/bbl")
    print(f"Estimasi fuel price terakhir: {latest['estimated_fuel_price_idr_per_liter']:,.2f} IDR/liter")
    print(f"Prediksi source bulan depan: {next_month['forecast_source_price_usd_per_bbl']:.2f} USD/bbl")
    print(f"Prediksi fuel price bulan depan: {next_month['forecast_fuel_price_idr_per_liter']:,.2f} IDR/liter")
