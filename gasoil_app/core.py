...
def forecast_mops(rows: Iterable[Row], months: int = 12, method: str = "damped_trend") -> list[Row]:
    ...
    recent_baseline = _median(values[-min(12, len(values)):])
    # Public proxy products such as EIA Heating Oil can have volatile long histories.
    # Keep point forecasts within a defensible floor based on recent prices so a
    # negative extrapolated trend does not become an unrealistic ~IDR 1/liter.
    point_floor = max(0.01, recent_baseline * 0.35)
    point_cap = max(point_floor, recent_baseline * 2.5)

    last_date = parse_date(monthly[-1][DATE_COLUMN]).replace(day=1)
    forecast: list[Row] = []
    for idx, value in enumerate(yhat, start=1):
        point = min(point_cap, max(point_floor, value))
...
def build_analysis(
    ...
    for row, estimate in zip(monthly, monthly_estimates, strict=True):
        row["source_price_usd_per_bbl"] = round(float(row[MOPS_COLUMN]), 6)
        row["estimated_fuel_price_idr_per_liter"] = round(estimate, 2)
    ...
    for row, estimate, low, high in zip(
        ...
        row["forecast_source_price_usd_per_bbl"] = round(float(row["forecast_mops_usd_per_bbl"]), 6)
        row["forecast_fuel_price_idr_per_liter"] = round(estimate, 2)
        row["estimated_idr_per_liter"] = round(estimate, 2)
