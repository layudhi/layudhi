...
PUBLIC_SOURCES: dict[str, PublicSource] = {
    "yahoo_heating_oil": PublicSource(
        ...
        description="Futures Heating Oil NYMEX (HO=F), produk distillate/minyak pemanas AS yang sering dipakai sebagai proxy diesel/gasoil; quote asli USD/gal dan dikonversi ke USD/bbl.",
    ),
    "yahoo_brent": PublicSource(
        ...
        description="Futures Brent crude oil (BZ=F), benchmark minyak mentah global; bukan produk gasoil/diesel, tetapi berguna sebagai proxy arah pasar energi; quote USD/bbl.",
    ),
    "yahoo_low_sulphur_gasoil": PublicSource(
        ...
        description="Futures ICE Low Sulphur Gasoil (LGO=F), proxy produk gasoil Eropa; quote asli USD/MT dan dikonversi ke USD/bbl.",
    ),
    "datahub_brent_daily": PublicSource(
        ...
        default_symbol=None,
        default_unit="usd_per_bbl",
        description="Harga spot Brent harian dari DataHub oil-prices public dataset; benchmark minyak mentah North Sea/global dalam USD/bbl.",
    ),
    "datahub_wti_daily": PublicSource(
        ...
        default_symbol=None,
        default_unit="usd_per_bbl",
        description="Harga spot WTI harian dari DataHub oil-prices public dataset; benchmark minyak mentah Amerika Serikat dalam USD/bbl.",
    ),
    "datahub_brent_monthly": PublicSource(
        ...
        default_symbol=None,
        default_unit="usd_per_bbl",
        description="Harga spot Brent bulanan dari DataHub oil-prices public dataset; benchmark minyak mentah North Sea/global dalam USD/bbl.",
    ),
    "datahub_wti_monthly": PublicSource(
        ...
        default_symbol=None,
        default_unit="usd_per_bbl",
        description="Harga spot WTI bulanan dari DataHub oil-prices public dataset; benchmark minyak mentah Amerika Serikat dalam USD/bbl.",
    ),
    "eia_ultra_low_sulfur_diesel_ny": PublicSource(
        ...
        default_unit="usd_per_gal",
        description="EIA DNav Spot Prices: Ultra-Low-Sulfur No. 2 Diesel di New York Harbor, Amerika Serikat; proxy produk diesel, quote asli USD/gal dan dikonversi ke USD/bbl.",
    ),
    "eia_ultra_low_sulfur_diesel_usgc": PublicSource(
        ...
        default_unit="usd_per_gal",
        description="EIA DNav Spot Prices: Ultra-Low-Sulfur No. 2 Diesel di U.S. Gulf Coast; proxy produk diesel, quote asli USD/gal dan dikonversi ke USD/bbl.",
    ),
    "eia_heating_oil_ny": PublicSource(
        ...
        default_unit="usd_per_gal",
        description="EIA DNav Spot Prices: No. 2 Heating Oil di New York Harbor, Amerika Serikat; produk distillate/minyak pemanas yang sering dipakai sebagai proxy diesel/gasoil, quote asli USD/gal dan dikonversi ke USD/bbl.",
    ),
    "eia_jet_fuel_usgc": PublicSource(
        ...
        default_unit="usd_per_gal",
        description="EIA DNav Spot Prices: Kerosene-Type Jet Fuel di U.S. Gulf Coast; proxy avtur/jet fuel, quote asli USD/gal dan dikonversi ke USD/bbl.",
    ),
    "orb_markets": PublicSource(
        ...
        default_symbol=None,
        default_unit="usd_per_gal",
        description="Scrape nilai indikatif halaman publik ORB Markets untuk benchmark energi seperti Heating Oil atau Jet Fuel A-1 Singapore MOPS bila tersedia di halaman publik.",
    ),
}
...
def _is_plausible_quote_value(value: float, quote_unit: str) -> bool:
    normalized = quote_unit.strip().lower()
    if normalized in {"usd_per_gal", "usd/gal", "gal"}:
        return 0 < value <= 20
    if normalized in {"usd_per_bbl", "usd/bbl", "bbl"}:
        return 0 < value <= 500
    if normalized in {"usd_per_mt", "usd/mt", "mt"}:
        return 0 < value <= 3_000
    return value > 0
...
def parse_eia_history_html(html_text: str, quote_unit: str = "usd_per_gal") -> list[Row]:
    ...
    for match in row_pattern.finditer(text):
        ...
        for raw_value in match.group("values").split():
            ...
            if _is_plausible_quote_value(value, quote_unit):
                rows_by_date[current] = convert_to_usd_per_bbl(value, quote_unit)
...
def describe_public_source(source: str) -> str:
    if source == "csv_url":
        return "CSV publik kustom. Pilih kolom tanggal/harga dan unit quote sesuai sumber data Anda."
    preset = PUBLIC_SOURCES.get(source)
    if not preset:
        return "Sumber tidak dikenal."
    symbol = f" Simbol/series default: {preset.default_symbol}." if preset.default_symbol else ""
    return f"{preset.label}. {preset.description}{symbol} Unit quote: {preset.default_unit}."
