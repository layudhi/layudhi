...
def test_parse_eia_history_html_ignores_implausible_gallon_values() -> None:
    html = "2026 May- 4 to May- 8 4.091 2026 4.114"

    rows = parse_eia_history_html(html)

    assert rows == [
        {"date": date(2026, 5, 4), "mops_usd_per_bbl": pytest.approx(4.091 * 42)},
        {"date": date(2026, 5, 6), "mops_usd_per_bbl": pytest.approx(4.114 * 42)},
    ]
...
def test_describe_public_source_explains_eia_heating_oil() -> None:
    description = describe_public_source("eia_heating_oil_ny")

    assert "No. 2 Heating Oil" in description
    assert "New York Harbor" in description
    assert "USD/gal" in description
